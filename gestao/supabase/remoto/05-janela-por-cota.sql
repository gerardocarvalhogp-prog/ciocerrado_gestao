-- =====================================================================
-- CORRECAO PARA O BANCO HOSPEDADO  ·  janela relativa por cota
--
-- NAO e uma migration. Ver LEIA-ME.md desta pasta.
-- Corresponde a migration local 20260824102500_janela_por_cota.sql —
-- corpo identico, so o cabecalho muda.
--
-- DUAS MUDANCAS PEDIDAS PELO ORGANIZADOR, sobre o 04
--
-- 1. A janela vira relativa: "48h depois que a anterior encerrar", em
--    vez de uma data fixa por cota. O relogio de cada cota comeca
--    quando a anterior fecha — por ter terminado de escolher ou por ter
--    estourado a janela.
--
-- 2. Prazo vencido TIRA da escolha. No 04 custava so a fila e as
--    reservas; agora a empresa atrasada esta fora daquela sessao.
--
-- CONTROLES
--
--   eventos.escolha_abre_em   quando a PRIMEIRA cota comeca a contar
--   cotas.janela_horas        horas daquela cota
--   cotas.prazo_indicacao     teto absoluto opcional (veio no 04)
--
-- Vale o que vier primeiro entre os tres. `escolha_abre_em` nulo = nada
-- expira, que e o estado atual do hospedado: as 6 cotas estao sem prazo
-- e o relogio so comeca quando alguem disser que comecou.
--
-- ATENCAO · quatro funcoes sao RECRIADAS
--
--   admin_listar_cotas    ganha janela_horas no retorno
--   admin_salvar_cota     ganha p_janela_horas (9 parametros)
--   admin_listar_eventos  ganha escolha_abre_em no retorno
--   admin_salvar_evento   ganha p_escolha_abre_em (12 parametros)
--
-- Sem o DROP viraria sobrecarga e a chamada antiga cairia na versao
-- velha, sem erro na tela. O admin.html desta leva ja manda os dois
-- parametros novos.
--
-- APLICAR
--   supabase db query --linked -f supabase/remoto/05-janela-por-cota.sql
--
-- ROLLBACK: as versoes anteriores estao no git, nas migrations
-- 20260824102400 (cotas/fila) e no dump 00-estado-anterior.sql.
-- =====================================================================
