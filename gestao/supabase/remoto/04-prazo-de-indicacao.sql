-- =====================================================================
-- CORRECAO PARA O BANCO HOSPEDADO  ·  indicacao vira reserva, com prazo
--
...
-- pasta).
--
-- Corresponde a migration local 20260824102400_prazo_de_indicacao.sql —
-- corpo identico, so o cabecalho muda.
--
-- A REGRA (definida pelo organizador em 24/08/2026)
--
--   1. Enquanto a Esmeralda esta no prazo dela, o convidado que ela
--      indicou no PERFIL nao aparece para mais ninguem.
--   2. A vez passa para a Diamante quando a Esmeralda termina de
--      escolher OU quando o prazo da cota dela vence.
--   3. Escolhido, o convidado fica preso na mesa de quem escolheu.
--   4. Prazo vencido sem escolha: a reserva cai e ele volta para a
--      lista geral.
--
-- Vencer o prazo custa a fila e as reservas, nao o direito de escolher
-- entre quem estiver livre.
--
-- O prazo fica em `cotas.prazo_indicacao`, uma data por cota. Cota sem
-- prazo se comporta como antes: segura a vez ate encerrar ou passar.
--
-- ATENCAO · duas funcoes sao RECRIADAS
--
--   admin_listar_cotas  ganha a coluna prazo_indicacao no retorno
--   admin_salvar_cota   ganha o parametro p_prazo_indicacao
--
-- Sem o DROP o Postgres criaria uma sobrecarga e as chamadas antigas
-- continuariam caindo na versao velha, sem erro nenhum na tela. O
-- admin.html desta mesma leva ja manda o parametro novo.
--
-- APLICAR
--   supabase db query --linked -f supabase/remoto/04-prazo-de-indicacao.sql
--
-- ROLLBACK: `supabase/remoto/00-estado-anterior.sql` tem as versoes de
-- patro_convidados_disponiveis anteriores a esta leva; para as demais,
-- as definicoes estao no git.
-- =====================================================================
